...
const ChoreProgressBar: React.FC<ChoreProgressBarProps> = ({
	...
	currentDate.setHours(0, 0, 0, 0) // Set time of day to 0 for current date
	const dueDateObj = new Date(dueDate)
	dueDateObj.setHours(0, 0, 0, 0) // Set time of day to 0 for due date
	...
	const progressPercentage = isChoreDue
		...
		? 0
		: ((choreRecurrenceInDays - daysUntilChoreIsDue) / choreRecurrenceInDays) *
		  100
	...
	return (
		<div>
			...
			{daysUntilChoreIsDue >= 0 ? (
				<p>
					Days until chore is due: {daysUntilChoreIsDue}{' '}
					{daysUntilChoreIsDue === 1 ? 'day' : 'days'}
				</p>
			) : (
				<p>
					Chore was due {Math.abs(daysUntilChoreIsDue)}{' '}
					{Math.abs(daysUntilChoreIsDue) === 1 ? 'day' : 'days'} ago
				</p>
			)}
		</div>
	)
